...
public class ByteCyclicBuffer implements Cloneable, ByteBuffer  {

    public ByteCyclicBuffer (int buffer_length) {
        this(new byte[buffer_length]);
    }
    // State
    // --- protected
    ...
    @Override
    public ByteCyclicBuffer append (final byte[] block, final int off, final int len) throws StorageException {
    ...
    private void p_appendBlock (final byte[] block, final int off, final int len) {
        assert p_invariablesHold();
    ...
    public String toString () {
        ...
        if (available > 0) {
            final String firstString = Integer.toHexString((int) buffer[0]);
            --available;
            ...
            while (available > 0) {
                strbld.append(", ")
                      .append(Integer.toHexString((int) buffer[i]));
                --available;
                i = (i+1) % buffer.length;
            }
     ...
     * @param buffer
     */
    private ByteCyclicBuffer (final byte[] _buffer) {
        buffer = _buffer;
        read_index = 0;
        store_index = 0;
